...
Extracted from `CLAUDE.md` to keep the always-loaded instructions lean. This is
a quick index; the **complete** per-function documentation lives in the
[`tic80wiki/`](../tic80wiki/) folder (local copy of the official TIC-80 wiki).
...
> For composing sound/music from Lua (not the editors), see the **tic80-sound** skill.
