...
-- Proof of concept: the demo reads the music clock to time visuals. This HUD is
-- throwaway (remove once real keyframing is hooked up) -- but keep Tracker.update().
local function drawMusicHud()
	local part = Tracker.partAt(Tracker.currentPartIndex())
	local onBeat = Tracker.row() % (Tracker.stepsPerBar() // 4) == 0
	rect(2, 2, 5, 5, onBeat and 15 or 1)                -- pulses on every beat
	print(part.name, 11, 2, 12, false, 1, true)
	print("row " .. Tracker.row() .. "  beat " .. Tracker.beat(), 11, 9, 13, false, 1, true)
	rect(2, 131, Tracker.channelLevel(3) * 4, 3, 9)     -- live kick level
end
...
function TIC()
	...
	drawMusicHud()            -- concept test: visuals driven by the music clock
end
